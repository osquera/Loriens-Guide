...
class TestVLMService(unittest.TestCase):
    ...
    def test_calculate_distance(self):
        ...
        distance = self.service._calculate_distance(lat1, long1, lat2, long2)
    ...
    def test_calculate_distance_same_point(self):
        ...
        distance = self.service._calculate_distance(lat, long, lat, long)
        
        self.assertEqual(distance, 0)
    
    def test_find_nearest_camera(self):
        ...
        self.assertEqual(nearest['camera_id'], 'lib_lobby_01')
    
    def test_find_nearest_camera_different_location(self):
        ...
        self.assertIsNotNone(nearest)
        self.assertEqual(nearest['camera_id'], 'lib_exit_01')
    
    def test_construct_vlm_prompt(self):
        """Test VLM prompt construction."""
        question = "Where is the exit?"
        context = "Library Lobby - Main Entrance, facing east"
        
        prompt = self.service._construct_vlm_prompt(question, context)
    ...
    def test_call_vlm_api_success(self, mock_post):
        """Test successful VLM API call."""
        # Mock successful response
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "text": "The exit is 20 steps forward."
        }
    ...
    @patch('vlm_service.requests.post')
    def test_call_vlm_api_error(self, mock_post):
    ...
    @patch('vlm_service.requests.post')
    def test_process_user_request(self, mock_post):
        """Test complete user request processing."""
        # Mock successful VLM response
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "text": "Walk straight ahead for 15 steps. The bathroom is on your left."
        }
        mock_post.return_value = mock_response
        
        # Process request
        result = self.service.process_user_request(
            lat=55.6761,
            long=12.5683,
